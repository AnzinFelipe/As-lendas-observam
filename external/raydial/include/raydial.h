...
typedef struct {
    ...
    bool animateText;                 // Enable text animation
    float animationSpeed;             // Characters per second (0.0-100.0)
    float elapsedTime;                // Time elapsed for animation
    int displayedCharCount;           // How many characters to display
    // Box style properties
    int borderWidth;                  // Border width
    Color borderColor;                // Border color
    int cornerRadius;                 // Corner radius for rounded boxes
    int padding;                      // Padding inside the box
...
typedef struct {
    ...
    bool isActive;
    void* userData;
    float deltaTime;                  // Delta time for animations
} RayDialManager;
