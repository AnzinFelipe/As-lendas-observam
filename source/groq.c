#include "groq.h"
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
#include <curl/curl.h>
 
#define GROQ_URL "https://api.groq.com/openai/v1/chat/completions"
#define GROQ_MODEL "llama-3.1-8b-instant"
 
// Leitura da chave (arquivo groq.key ou variável GROQ_API_KEY — mesmo arquivo, só muda o conteúdo)
static int ler_chave_api(char *dest, int dest_size) {
    const char *env = getenv("GROQ_API_KEY");
    if (env && strlen(env) > 0) {
        strncpy(dest, env, dest_size - 1);
        dest[dest_size - 1] = '\0';
        return 1;
    }
    FILE *f = fopen("groq.key", "r");
    if (!f) return 0;
    if (fgets(dest, dest_size, f)) {
        dest[strcspn(dest, "\r\n")] = '\0';
        fclose(f);
        return strlen(dest) > 0;
    }
    fclose(f);
    return 0;
}
 
// Buffer para acumular a resposta HTTP
typedef struct {
    char *data;
    size_t tamanho;
} CurlBuffer;
 
static size_t curl_write_cb(void *conteudo, size_t size, size_t nmemb, void *userp) {
    size_t total = size * nmemb;
    CurlBuffer *buf = (CurlBuffer *)userp;
    char *novo = realloc(buf->data, buf->tamanho + total + 1);
    if (!novo) return 0;
    buf->data = novo;
    memcpy(buf->data + buf->tamanho, conteudo, total);
    buf->tamanho += total;
    buf->data[buf->tamanho] = '\0';
    return total;
}
 
// Extrai o texto da resposta JSON do Groq
// Formato: {"choices":[{"message":{"content":"TEXTO"}}]}
static void extrair_texto(const char *json, char *dest, int dest_size) {
    const char *marcador = "\"content\":";
    const char *p = strstr(json, marcador);
    if (!p) {
        strncpy(dest, "As lendas te observam em silencio...", dest_size - 1);
        dest[dest_size - 1] = '\0';
        return;
    }
    p += strlen(marcador);
    while (*p == ' ' || *p == '\t' || *p == '\n' || *p == '\r') p++;
    if (*p != '"') {
        strncpy(dest, "As lendas te observam em silencio...", dest_size - 1);
        dest[dest_size - 1] = '\0';
        return;
    }
    p++;
 
    int i = 0;
    while (*p && i < dest_size - 1) {
        if (*p == '\\') {
            p++;
            if      (*p == '"')  { dest[i++] = '"';  }
            else if (*p == 'n')  { dest[i++] = ' ';  }
            else if (*p == 'r')  { dest[i++] = ' ';  }
            else if (*p == 't')  { dest[i++] = ' ';  }
            else if (*p == '\\') { dest[i++] = '\\'; }
            else                 { dest[i++] = *p;   }
            if (*p) p++;
            continue;
        }
        if (*p == '"') break;
        dest[i++] = *p++;
    }
    dest[i] = '\0';
}
 
// Thread
typedef struct {
    GroqContext *ctx;
    char prompt[1024];
} ThreadArgs;
 
static void *groq_thread_func(void *arg) {
    ThreadArgs *args = (ThreadArgs *)arg;
    GroqContext *ctx = args->ctx;
 
    char resposta_local[GROQ_RESPOSTA_MAX] =
        "Ei....";
 
    char api_key[256] = "";
    bool sucesso = false;
    if (!ler_chave_api(api_key, sizeof(api_key))) goto finalizar;
 
    {
        // Corpo JSON no formato OpenAI (compatível com Groq)
        char body[2048];
        snprintf(body, sizeof(body),
            "{"
            "\"model\":\"%s\","
            "\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}],"
            "\"max_tokens\":80,"
            "\"temperature\":0.9"
            "}",
            GROQ_MODEL, args->prompt);
 
        // Header com autenticação Bearer
        char auth_header[512];
        snprintf(auth_header, sizeof(auth_header), "Authorization: Bearer %s", api_key);
 
        CurlBuffer buf = {NULL, 0};
        buf.data = malloc(1);
        buf.data[0] = '\0';
 
        CURL *curl = curl_easy_init();
        if (curl) {
            struct curl_slist *headers = NULL;
            headers = curl_slist_append(headers, "Content-Type: application/json");
            headers = curl_slist_append(headers, auth_header);
 
            curl_easy_setopt(curl, CURLOPT_URL, GROQ_URL);
            curl_easy_setopt(curl, CURLOPT_POSTFIELDS, body);
            curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, curl_write_cb);
            curl_easy_setopt(curl, CURLOPT_WRITEDATA, &buf);
            curl_easy_setopt(curl, CURLOPT_TIMEOUT, 10L);
 
            CURLcode res = curl_easy_perform(curl);
            long http_code = 0;
            curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &http_code);
 
            if (res == CURLE_OK && http_code == 200 && buf.data) {
                extrair_texto(buf.data, resposta_local, GROQ_RESPOSTA_MAX);
                sucesso = true;
            } else if (http_code == 429) {
                snprintf(resposta_local, sizeof(resposta_local),
                         "Erro: limite de requisicoes atingido.");
            } else {
                snprintf(resposta_local, sizeof(resposta_local),
                         "Erro HTTP %ld", http_code);
            }
 
            curl_slist_free_all(headers);
            curl_easy_cleanup(curl);
        }
        free(buf.data);
    }
 
finalizar:
    pthread_mutex_lock(&ctx->mutex);
    strncpy(ctx->resposta, resposta_local, GROQ_RESPOSTA_MAX - 1);
    ctx->resposta[GROQ_RESPOSTA_MAX - 1] = '\0';
    ctx->status = sucesso ? GROQ_PRONTO : GROQ_ERRO;
    pthread_mutex_unlock(&ctx->mutex);
    free(args);
    return NULL;
}
 
// API pública
 
void GroqInit(GroqContext *ctx) {
    memset(ctx, 0, sizeof(GroqContext));
    ctx->status = GROQ_IDLE;
    pthread_mutex_init(&ctx->mutex, NULL);
    curl_global_init(CURL_GLOBAL_DEFAULT);
}
 
void GroqFree(GroqContext *ctx) {
    pthread_mutex_lock(&ctx->mutex);
    GroqStatus s = ctx->status;
    pthread_mutex_unlock(&ctx->mutex);
    if (s == GROQ_LOADING) pthread_join(ctx->thread, NULL);
    pthread_mutex_destroy(&ctx->mutex);
    curl_global_cleanup();
}
 
bool GroqPedirDialogo(GroqContext *ctx, const char *ultima_lenda) {
    pthread_mutex_lock(&ctx->mutex);
    GroqStatus s = ctx->status;
    pthread_mutex_unlock(&ctx->mutex);
    if (s == GROQ_LOADING) return false;
 
    ThreadArgs *args = malloc(sizeof(ThreadArgs));
    if (!args) return false;
 
    args->ctx = ctx;
    snprintf(args->prompt, sizeof(args->prompt),
        "Não seja pejorativo, seja ameaçador e severo. "
        "Voce é o Boca de Ouro, uma lenda urbana do Recife Antigo. "
        "Voce esta observando uma pessoa que esta perdida a um tempo."
        "Ela encontrou e falou com %s recentemente e você estava vendo de longe."
        "Fale em 2 frases curtas,"
        "em portugues do Brasil de forma formal e arcaica,"
        "tom ameacador e severo, como se voce tivesse seguindo ele desde muito cedo, cite quem ele falou recentemente. "
        "Responda APENAS o dialogo em si, sem aspas, sem travessao, sem narracao, sem introducao.",
        ultima_lenda && strlen(ultima_lenda) > 0 ? ultima_lenda : "nenhuma ainda");
 
    pthread_mutex_lock(&ctx->mutex);
    ctx->status = GROQ_LOADING;
    pthread_mutex_unlock(&ctx->mutex);
 
    if (pthread_create(&ctx->thread, NULL, groq_thread_func, args) != 0) {
        pthread_mutex_lock(&ctx->mutex);
        ctx->status = GROQ_ERRO;
        pthread_mutex_unlock(&ctx->mutex);
        free(args);
        return false;
    }
    pthread_detach(ctx->thread);
    return true;
}
 
GroqStatus GroqGetStatus(GroqContext *ctx) {
    pthread_mutex_lock(&ctx->mutex);
    GroqStatus s = ctx->status;
    pthread_mutex_unlock(&ctx->mutex);
    return s;
}
 
void GroqPegarResposta(GroqContext *ctx, char *dest, int dest_size) {
    pthread_mutex_lock(&ctx->mutex);
    strncpy(dest, ctx->resposta, dest_size - 1);
    dest[dest_size - 1] = '\0';
    ctx->status = GROQ_IDLE;
    pthread_mutex_unlock(&ctx->mutex);
}